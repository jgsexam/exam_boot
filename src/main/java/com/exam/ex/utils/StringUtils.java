package com.exam.ex.utils;

import com.exam.ex.constant.CharConstant;
...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
     ...
     * @return
     */
    public static boolean isPictureExt(String ext) {
        boolean flag;
        // 先判断是否为空，不为空再继续
        flag = isNotBlank(ext);
        if (flag) {
            if(!ExtConstant.JPG_EXT.equals(ext.toLowerCase())) {
                flag = false;
            }
            if(!ExtConstant.JPEG_EXT.equals(ext.toLowerCase())) {
                flag = false;
            }
            if(!ExtConstant.PNG_EXT.equals(ext.toLowerCase())) {
                flag = false;
            }
        }
        return flag;
     ...
     * @return
     */
    public static boolean isNotPictureExt(String ext) {
        boolean flag;
        // 先判断是否为空，不为空再继续
        flag = isBlank(ext);
        if (!flag) {
